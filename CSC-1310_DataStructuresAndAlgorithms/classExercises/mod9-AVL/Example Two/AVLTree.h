...
/*	
	insert:	Insert a new node into the AVL tree with value	
			...
			In the first call from calling funciton, node is the root
...
TreeNode *AVLTree::insert(TreeNode *node, int num)
{
    ...
    else if (num < node->getValue()) //move to the left
    {
		//the insert part in the statement below will allow to create a new node on the left
        node->setLeft(insert(node->getLeft(), num));
        node = balance(node);
    }
    else if (num >= node->getValue()) //move to the right
    {
