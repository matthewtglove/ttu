/*
	Title:  	arrayFunc_1.cpp
	Author: 	April Crockett
	Date:   	3-1-2022
	Purpose: 	This program uses a function to double the value of
...
void doubleArray(int nums[], int size)
{
   for (int index = 0; index < size; index++)
      nums[index] *= 2; //could also be written nums[index]=nums[index]*2;
  
   /*
      NOTE: Arrays get passed by reference without using &
	  This means that any change you make to the array elements
	  in this function are applied to the set array in the main function.
   */
}
